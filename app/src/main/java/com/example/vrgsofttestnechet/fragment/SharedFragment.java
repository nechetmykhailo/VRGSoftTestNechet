...
public class SharedFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rwShared = v.findViewById(R.id.rwShared);
        models = new ArrayList<>();

        mLayoutManagers = new LinearLayoutManager(getContext());
        rwShared.setLayoutManager(mLayoutManagers);

        mAdapter = new RecyclerAdapterNews(getContext(), models, Constants.KEY_ALL);
        rwShared.setAdapter(mAdapter);

        metodGetRetrofitEmailed();

        rwShared.getAdapter().notifyDataSetChanged();
        return v;
    }

    private void metodGetRetrofitEmailed() {
        Call<JsonObject> call = api.getShared(Constants.API_KEYS);
